...
const parseLine = (textLine) => 
{
	...
	if (particles) 
	{
		...
		let wrappedText = wordWrap(preface, COLS - INDENT - link.length, LINEBR)
                          .split(LINEBR);
		...
		else 
		{
			...
			wrappedText = wordWrap(
                joinedText, COLS - INDENT_SUB - link.length, LINEBR
            ).split(LINEBR);

			wrappedText.forEach(function(elem, idx, arr) 
			{
                // Make sure we haven't reached the last line
				if (idx !== arr.length - 1)
				{
				...
				// Inject dots when on the last line
				const dots = ".".repeat(
                    COLS - INDENT_SUB - elem.length - link.length - 1);
